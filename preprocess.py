...
def prepare(in_dir = 'Liver', pixdim = (1.5, 1.5, 1.0), a_min = - 200, a_max = 200, spatial_size = [128, 128, 64], cache = False):
    """
    ...
    path_test_segmentation = sorted(glob(os.path.join(in_dir, 'nifti_files', 'TestSegmentation', '*nii.gz')))
    ...
    train_transforms = Compose([
        ...
        Spacingd(keys = ['vol', 'seg'], pixdim = pixdim, mode = ('bilinear', 'nearest')), # bilinear interpoliert das vol, nearest das seg
        Orientationd(keys = ['vol', 'seg'], axcodes = 'RAS'),# RAS sorgt dafür das Beide Datenschlüssel die gleichen Achsen haben
        ScaleIntensityRanged(keys = ['vol'], a_min = a_min, a_max = a_max, b_min = 0.0, b_max = 1.0, clip = True),
        CropForegroundd(keys = ['vol', 'seg'], source_key = 'vol' ),
        Resized(keys = ['vol', 'seg'], spatial_size = spatial_size), #Pixel größer erhöht die genauigkeit, kostet mehr Ressourcen
        ToTensord(keys = ['vol', 'seg'])
        
    ])
    
    test_transforms = Compose([
        LoadImaged(keys=["vol", "seg"], image_only = False),
        EnsureChannelFirstd(keys=["vol", "seg"]),
